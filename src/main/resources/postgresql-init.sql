...
create table if not exists public.release_item_master
(
    id               serial    not null
        constraint pk_release_item_master
            primary key,
    created_at       timestamp not null,
    last_modified_at timestamp not null,
    master_id        integer   not null
        constraint fk_release_item_master_master_id_master
            references public.master,
    release_item_id  integer   not null
        constraint fk_release_item_master_release_item_id_release_item
            references public.release_item,
    constraint uq_release_item_master_release_item_id_master_id
        unique (release_item_id, master_id)
);
